...
4) In both "extractor.config.json" files set property "recordingPath" to absolute path of MCAP recordings:
...
6) In both "generator.config.json" files set property "import.dataRoot" to the path of this new folder
7) Adjust BLENDER_PATH in "run-generator.bat" (optional: remove "--background" flag for GUI)
...
Recommendation:
- Enable GPU Support in Blender before starting the Render (https://docs.blender.org/manual/en/4.5/render/cycles/gpu_rendering.html)
